...
description: NPM 배포를 처음 해보는 사람들을 위해서 차근차근 가이드를 작성해봤습니다.
thumbnail: ./cover.png
...
![우리가 작성한 index.js파일이 node_modules안에 잘 들어가있다.](./1.png)
...
`exports` 필드는 [node v12.7.0 버전에 추가된 필드로](https://nodejs.org/api/packages.html#exports), `cjs`와 `esm`을 동시에 지원할 수 있게 해준다.
...
하지만 `cjs` 환경에서는 `require`를 사용하기 때문에 `require`를 키로 가지는 객체 안에 `index.cjs` 파일을 바라보게 해야한다.

`commonjs`를 사용하는 `index.cjs`를 작성하고, 프로그램 파일이 많아지니 소스파일을 `src` 폴더를 만들어 그 안에 넣어주자.
...
타입스크립트 패키지 자체는 빌드 결과물에는 포함되지 않아도 되기 때문에 `devDependencies`에 설치해준다.
...
그리고 배포하기 전에 확실히 `build` 명령어를 실행하고 배포를 하기 위해 `prepack` 스크립트를 `package.json`에 추가해주자.
