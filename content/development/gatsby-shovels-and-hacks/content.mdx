...
![gatsby-plugin-image는 귀찮은 것들을 해결해 준다.](./image-plugin-is-good.webp)
...
![placeholder에 대한 여러 옵션들](./placeholder-option.webp)
...
![자동 생성된 타입은 GatsbyTypes에 담겨있어요.](./typegen-1.webp)
...
![캐시만 제대로 해줘도 많이 빨라진 모습](./ci-time.webp)
...
![slice 과정을 도식화 [출처 Gatsby 공식문서]](./using-slices-build.webp)
...
![slice API 에러 발생](./slice-ssr-error.webp)
...
![slice API 핫픽스](./resolve-error.webp)
...
![cloudflare는 node 18 버전을 지원하지 않아요](./cloudflare-ci-node-version.webp)
...
![vercel은 작년 2022년 12월에 node 18 버전을 지원해요](./vercel-ci-node-version.webp)
